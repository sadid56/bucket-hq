  ...
  return (
    <Stack gap={6}>
      <Flex align="center" justify="space-between" direction={{ base: "column", sm: "row" }} gap={4}>
        <Stack gap={1}>
          <Heading size="lg" fontWeight="bold">User Account Administration</Heading>
          <Text fontSize="sm" color="fg.muted">Audit global logins, ban status, and active sessions</Text>
        </Stack>
        <Input
      ...
      <DataTable
        ...
        renderRow={(user: User) => (
          <tr key={user.id} style={{ borderBottom: "1px solid var(--chakra-colors-border-subtle)" }}>
            ...
            <td style={{ padding: "12px" }}>
              <Text fontSize="xs" color="fg.muted">{new Date(user.createdAt).toLocaleDateString()}</Text>
            </td>
            <td style={{ padding: "12px" }}>
              <Button
                size="xs"
                variant="outline"
                colorPalette={user.banned ? "green" : "red"}
        ...
        >
          ...
          >
            <Flex justify="space-between" align="center" borderBottomWidth="1px" pb={3}>
              <Heading size="md" fontWeight="bold">Active Login Sessions</Heading>
              ...
              renderRow={(sess: Session) => (
                <tr key={sess.id} style={{ borderBottom: "1px solid var(--chakra-colors-border-subtle)" }}>
                  <td style={{ padding: "8px 12px" }}>
                    <Text fontSize="xs" fontFamily="mono">{sess.ipAddress || "-"}</Text>
                  </td>
                  <td style={{ padding: "8px 12px" }}>
                  ...
                  <td style={{ padding: "8px 12px" }}>
                    <Text fontSize="2xs" color="fg.muted">{new Date(sess.createdAt).toLocaleString()}</Text>
                  </td>
                </tr>
              )}
            />
        ...
        >
          ...
          >
            ...
            <Stack direction="row" justify="flex-end" gap={3} mt={2}>
              ...
              <Button
                size="sm"
                colorPalette={banUser.banned ? "green" : "red"}
                loading={togglingBan}
