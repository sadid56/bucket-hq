...
export function GlobalAuditLogs() {
  ...
  const { data: logs = [], isLoading: loading, refetch } = useAudits({
    action,
    userId,
    startDate,
    endDate,
  ...
  return (
    <Stack gap={6}>
      <Stack gap={1}>
        <Heading size="lg" fontWeight="bold">SaaS Immutable Audit Trails</Heading>
        <Text fontSize="sm" color="fg.muted">Verify all pre-sign request payloads and storage updates</Text>
      </Stack>
      ...
      <Box bg="bg.panel" p={4} borderRadius="lg" borderWidth="1px" shadow="sm">
        <Flex gap={4} wrap="wrap" align="flex-end">
          <Stack gap={1} minW="160px" flex="1">
            <Text fontSize="xs" fontWeight="semibold" color="fg.muted">Action Filter</Text>
            <SelectRoot
          ...
          <Stack gap={1} minW="160px" flex="1">
            <Text fontSize="xs" fontWeight="semibold" color="fg.muted">Start Date</Text>
            <Input
              type="date"
              size="sm"
              height="36px"
          ...
          <Stack gap={1} minW="160px" flex="1">
            <Text fontSize="xs" fontWeight="semibold" color="fg.muted">End Date</Text>
            <Input
              type="date"
              size="sm"
              height="36px"
        ...
        renderRow={(log: AuditLog) => (
          <tr key={log.id} style={{ borderBottom: "1px solid var(--chakra-colors-border-subtle)" }}>
            <td style={{ padding: "10px 12px" }}>
              <Text fontWeight="semibold" fontSize="sm">{log.user.name}</Text>
              <Text fontSize="2xs" color="fg.muted">{log.user.email}</Text>
            </td>
            <td style={{ padding: "10px 12px" }}>
              <Text fontSize="xs" fontFamily="mono">{log.ipAddress || "-"}</Text>
            </td>
            <td style={{ padding: "10px 12px" }}>
              <Badge colorPalette={getActionBadgeColor(log.action)}>{log.action}</Badge>
            </td>
            <td style={{ padding: "10px 12px" }}>
              <Text fontSize="xs" fontFamily="mono" maxW="240px" truncate title={log.objectPath}>
                {log.objectPath || "-"}
              </Text>
            </td>
            <td style={{ padding: "10px 12px" }}>
              ...
              {log.storageConnection && (
                <Text fontSize="2xs" color="fg.muted">{log.storageConnection.providerType}</Text>
              )}
            </td>
            <td style={{ padding: "10px 12px" }}>
              <Text fontSize="2xs" color="fg.muted">{new Date(log.createdAt).toLocaleString()}</Text>
            </td>
          </tr>
        )}
      />
